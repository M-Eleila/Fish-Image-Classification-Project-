...
batch_size = 32
workers = 64
architecture = 'alexnet_for_robo_freezed'
if(unfreeze_trained_layers):
    architecture = 'alexnet_for_robo_freezed'
...
number_features = trained_model.classifier[6].in_features
features = list(trained_model.classifier.children())[:-1] # Remove last layer
features.extend([torch.nn.Linear(number_features, 30, device="cuda")])
trained_model.classifier = torch.nn.Sequential(*features)

print("****************** STEP2.2: laod the weights  ******************")
checkpoint = torch.load("/home/u867803/bestModels/1_2_Alex_Affine/alex_model_best_sbatchqueued.pth")
...
# Modify the last layer
number_features = model_for_robo.classifier[6].in_features
features = list(model_for_robo.classifier.children())[:-1] # Remove last layer
features.extend([torch.nn.Linear(number_features, number_of_Robo_classes, device="cuda")])
model_for_robo.classifier = torch.nn.Sequential(*features)
...
for epoch in range(0, epochs):
    ...
    #print("here {0}", 1)
    #**************** train for one epoch************************#
    train_running_corrects, train_running_loss = finalAlexNetModelpy.train_epoch(train_loader, model_for_robo, criterion, optimizer, epoch, print_freq)
    ...
    val_running_corrects, val_running_loss = finalAlexNetModelpy.validate_epoch(val_loader, model_for_robo, criterion, print_freq)
