...
batch_size = 32
workers = 64
architecture = 'convnext_for_affine'
learning_rate = 0.01
epochs = 90
print_freq = 500
best_acc = 0.00
...
checkpoint = torch.load("/home/u867803/Projects/Thesis/ModelResults/20221126_ConvNext_ImageNet/checkpoint-best.pth")
...
#features.extend([torch.nn.Linear(number_features, number_of_Affine_classes, **kwargs)])
features.extend([new_layer])
model_for_affine.head = torch.nn.Sequential(*features)

#model_for_affine.head = nn.Sequential(nn.Linear(768, number_of_Affine_classes, device="cuda"))

for param in model_for_affine.head.parameters():
    param.requires_grad = True

# Unfreeze the last stage
for param in model_for_affine.stages[3].parameters():
    param.requires_grad = True
...
for epoch in range(0, epochs):
    ...
    #print("here {0}", 1)
    #**************** train for one epoch************************#
    train_running_corrects, train_running_loss = ModelExecutionHelper.train_epoch(train_loader, model_for_affine, criterion, optimizer, epoch, print_freq)
    ...
    val_running_corrects, val_running_loss = ModelExecutionHelper.validate_epoch(val_loader, model_for_affine, criterion, print_freq)
